...
public class PlanForgeEvents {
    ...
    public static final Event<OnMove> ON_MOVE = EventFactory.createArrayBacked(OnMove.class, callbacks -> (handler, packet) -> {
        for (OnMove callback : callbacks) {
            ...
            if (ForgePlayerPositionTracker.moved(playerUUID, packet.getX(x), packet.getY(y), packet.getZ(z), packet.getYaw(yaw), packet.getPitch(pitch))) {
     ...
     * only call {@link PlanForge#isSystemEnabled} to determine if the enable was successful.</strong>
    ...
    public interface OnEnable {
        void onEnable(PlanForge plugin);
    }
}
